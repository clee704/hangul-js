...
function applyFullWidthStyle(text) {
    ...
    for (i = 0; i < text.length; i++) {
        ...
        c = (c === '\\' ? '\uffe6' : ('!' <= c && c <= '~') ? String.fromCharCode(c.charCodeAt(0) + 0xfee0) : c);
        buffer.push(c);
    }
    return buffer.join('');
}
...
function _shift(text, layout, reverse) {
    ...
    for (i = 0; i < text.length; i++) {
        ...
        c = text.charAt(i);
        q = layout.toQwerty(c);
        x = (c !== q || layout.map.hasValue(c) ? layout.fromQwerty(_upperStr(q, reverse)): _upper(c, reverse));
        buffer.push(x);
    }
    return buffer.join('');
}
...
function _upper(c, reverse) {
    return !reverse ? qwertyShiftMap.get(c) || c : qwertyShiftMap.inverse.get(c) || c;
}
...
function _tugInitials(buffer, text, i, c0, c1, c2, c3) {
    ...
    d = hangul.composeDoubleJamo(c2, c3);
    if (!hangul.isFinal(d) || (hangul.isMedial(text.charAt(i + 4)) && (c3 === '\u314e' || (c2 === c3 && (c2 === '\u3131' || c2 === '\u3145'))))) {
...
function _isOkToTug(c2, c3) {
    return !hangul.isMedial(c3) || (!hangul.isIotizedVowel(c3) || c2 !== '\u3137' && c2 !== '\u314c') && c2 !== '\u3147' && c2 !== '\u314e';
}
...
function nudgeFinals(text) {
    ...
    for (i = 0; i < buffer.length; i++) {
        jamo0 = hangul.decompose(buffer[i]);
        jamo1 = hangul.decompose(buffer[i + 1]);
        if (!jamo0 || !jamo1) {
            continue;
        }
        fin = jamo0[2];
        ...
        if (_isOkToNudge(fin, ini, med)) {
            cc = hangul.decomposeDoubleJamo(fin);
        ...
        d = hangul.composeDoubleJamo(fin, ini);
...
function _isOkToNudge(fin, ini, med) {
    return fin !== '\u3147' && fin !== '\u314e' && fin !== '\u3136' && fin !== '\u3140' && ini === '\u3147' &&
