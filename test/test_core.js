...
function test_Map() {
    ...
    assert.that(map.get('R'), eq('Romeo'));
    assert.that(map.get('J'), eq('Juliett'));
    assert.that(map.hasKey('Z'), isTrue());
    assert.that(map.inverse.get('Foxtrot'), eq('F'));
    assert.that(map.inverse.get('Zulu'), eq('Z'));
    assert.that(map.inverse.hasValue('Romeo'), isFalse());
    assert.that(map.get('Victor'), eq(undefined));
},
